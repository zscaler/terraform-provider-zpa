...
func dataSourceGlobalPolicyIsolation() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceGlobalPolicyIsolationRead,
...
func dataSourceGlobalPolicyIsolationRead(d *schema.ResourceData, m interface{}) error {
	zClient := m.(*Client)
	log.Printf("[INFO] Getting data for global isolation policy\n")

	resp, _, err := zClient.policysetglobal.GetIsolationPolicy()
	if err != nil {
		return err
	}
	...
	if err := d.Set("rules", flattenPolicyIsolation(resp)); err != nil {
		return err
	}
...
func flattenPolicyIsolation(policyIsolationRules *policysetglobal.PolicySet) []interface{} {
	ruleItems := make([]interface{}, len(policyIsolationRules.Rules))
	for i, ruleItem := range policyIsolationRules.Rules {
		ruleItems[i] = map[string]interface{}{
			...
			"conditions":                  flattenIsolationRuleConditions(ruleItem),
		}
	}
...
func flattenIsolationRuleConditions(conditions policysetglobal.Rules) []interface{} {
	...
	for i, ruleCondition := range conditions.Conditions {
		ruleConditions[i] = map[string]interface{}{
			...
			"operands":      flattenIsolationConditionOperands(ruleCondition),
		}
	}
...
func flattenIsolationConditionOperands(operands policysetglobal.Conditions) []interface{} {
