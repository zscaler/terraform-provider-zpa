...
func testAccCheckPolicyTimeoutRuleV2Destroy(s *terraform.State) error {
	...
	accessPolicy, _, err := policysetcontrollerv2.GetByPolicyType(context.Background(), apiClient.Service, "TIMEOUT_POLICY")
	if err != nil {
...
func testAccCheckPolicyTimeoutRuleV2Exists(resource string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		...
		resp, _, err := policysetcontrollerv2.GetByPolicyType(context.Background(), apiClient.Service, "TIMEOUT_POLICY")
		if err != nil {
			return fmt.Errorf("failed fetching resource TIMEOUT_POLICY. Recevied error: %s", err)
		}
