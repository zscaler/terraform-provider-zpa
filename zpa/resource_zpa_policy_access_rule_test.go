...
func TestAccResourcePolicyAccessRule(t *testing.T) {

	rName := acctest.RandString(10)
	rDesc := acctest.RandString(20)
	resourceName := "zpa_policy_access_rule.policy_access_test"
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceZPAPolicyAccessRuleConfigBasic(rName, rDesc),
				Check: resource.ComposeTestCheckFunc(
					tesAccCheckPolicyAccessRuleExists(resourceName),
...
func testAccResourceZPAPolicyAccessRuleConfigBasic(rName, rDesc string) string {
	return fmt.Sprintf(`
...
resource "zpa_policy_access_rule" "policy_access_test" {
	name                          = "%s"
	description                   = "%s"
	action                        = "ALLOW"
	rule_order                    = 1
...
func tesAccCheckPolicyAccessRuleExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("Policy Access Rule Not found: %s", n)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("no Policy Access Rule ID is set")
		}
