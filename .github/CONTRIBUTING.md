# Contributing to Terraform - ZPA Provider
...
- [ ] **Search for answers in Terraform documentation**: We're happy to answer
      ...
      documentation](https://www.terraform.io/docs/providers/zpa/index.html). Oftentimes Question issues result in documentation updates
...
- [ ] _Resource Acceptance Testing Implementation_: In the resource acceptance testing (e.g. `okta/resource_zpa_service_thing_test.go`), implementation of `TestStep`s with `ImportState: true`
...
- [ ] **Resource Naming**: Resources should be named `zpa_<service>_<name>`,
      ...
      the resource naming with the service for [Customizing Endpoints](https://www.terraform.io/docs/providers/zpa/guides/custom-service-endpoints.html#available-endpoint-customizations),
...
- [ ] **Uses ZPA Go SDK Types**: Use available SDK structs instead of implementing custom types with indirection.
...
- [ ] **Skips Error() Call with ZPA Go SDK Error Objects**: Error objects do not need to have `Error()` called.
...
The below are location-based items that _may_ be noted during review and are recommended for consistency with testing flexibility. Resource testing is expected to pass across multiple ZPA environments supported by the Terraform ZPA Provider (e.g. ZPA Standard and ZPA GovCloud (US)). Contributors are not expected or required to perform testing outside of ZPA Standard, e.g. running only in the `us-west-2` region is perfectly acceptable, however these are provided for reference:
...
4. The resources created by the test are destroyed. This step happens
   automatically, and is the equivalent of calling `terraform destroy`.

5. Assertions are made against the provider API to verify that the resources
...
[website]: https://github.com/zscaler/terraform-provider-zpa/docs
