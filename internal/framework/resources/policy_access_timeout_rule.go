...
var policyTimeoutImportTypes = []string{helpers.PolicyTypeTimeout, helpers.PolicyTypeReauth}

var (
	_ resource.Resource                = &PolicyAccessTimeoutRuleResource{}
	_ resource.ResourceWithConfigure   = &PolicyAccessTimeoutRuleResource{}
	_ resource.ResourceWithImportState = &PolicyAccessTimeoutRuleResource{}
)

func NewPolicyAccessTimeoutRuleResource() resource.Resource {
	return &PolicyAccessTimeoutRuleResource{}
}

type PolicyAccessTimeoutRuleResource struct {
	client *client.Client
}

type PolicyAccessTimeoutRuleModel struct {
...
func (r *PolicyAccessTimeoutRuleResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_policy_access_timeout_rule"
}

func (r *PolicyAccessTimeoutRuleResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		Description: "Manages ZPA timeout and re-auth policy rules.",
...
func (r *PolicyAccessTimeoutRuleResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *PolicyAccessTimeoutRuleResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	if r.client == nil {
		resp.Diagnostics.AddError("Unconfigured Provider", "The provider must be configured before managing timeout policy rules.")
		return
	}

	var plan PolicyAccessTimeoutRuleModel
	...
	payload, diags := expandPolicyAccessTimeoutRule(ctx, &plan, policySetID)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Failed to create timeout policy rule: %v", err))
		return
	}
...
func (r *PolicyAccessTimeoutRuleResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	if r.client == nil {
		resp.Diagnostics.AddError("Unconfigured Provider", "The provider must be configured before managing timeout policy rules.")
		return
	}

	var state PolicyAccessTimeoutRuleModel
...
func (r *PolicyAccessTimeoutRuleResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	if r.client == nil {
		resp.Diagnostics.AddError("Unconfigured Provider", "The provider must be configured before managing timeout policy rules.")
		return
	}
	...
	if _, err := policysetcontroller.UpdateRule(ctx, service, policySetID, plan.ID.ValueString(), payload); err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Failed to update timeout policy rule: %v", err))
		return
	}
...
func (r *PolicyAccessTimeoutRuleResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	if r.client == nil {
		resp.Diagnostics.AddError("Unconfigured Provider", "The provider must be configured before managing timeout policy rules.")
		return
	}
	...
	if _, err := policysetcontroller.Delete(ctx, service, policySetID, helpers.StringValue(state.ID)); err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Failed to delete timeout policy rule: %v", err))
		return
	}
}

func (r *PolicyAccessTimeoutRuleResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	if r.client == nil {
		resp.Diagnostics.AddError("Unconfigured Provider", "The provider must be configured before importing timeout policy rules.")
		return
	}

	id := strings.TrimSpace(req.ID)
	if id == "" {
		resp.Diagnostics.AddError("Invalid Import ID", "Import requires the timeout policy rule ID or name.")
		return
	}
	...
	rule, _, err := policysetcontroller.GetByNameAndTypes(ctx, r.client.Service, policyTimeoutImportTypes, id)
	if err != nil {
		resp.Diagnostics.AddError("Import Error", fmt.Sprintf("Unable to locate timeout policy rule %q: %v", id, err))
		return
	}
...
func (r *PolicyAccessTimeoutRuleResource) readPolicyAccessTimeoutRule(ctx context.Context, service *zscaler.Service, policySetID, ruleID string, microtenantID types.String) (PolicyAccessTimeoutRuleModel, diag.Diagnostics) {
	...
	if err != nil {
		if errResp, ok := err.(*errorx.ErrorResponse); ok && errResp.IsObjectNotFound() {
			return PolicyAccessTimeoutRuleModel{}, diag.Diagnostics{diag.NewErrorDiagnostic("Not Found", fmt.Sprintf("Timeout policy rule %s not found", ruleID))}
		}
...
func expandPolicyAccessTimeoutRule(ctx context.Context, model *PolicyAccessTimeoutRuleModel, policySetID string) (*policysetcontroller.PolicyRule, diag.Diagnostics) {
